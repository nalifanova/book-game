...
{
    log::ok("Welcome to " + msg::k_title + " " + msg::k_book_name);
    log::ok(std::string(row::k_underline_length + 10, '-'));
}
...
void l_print_attributes(int& dexterity, int& hp, int& charisma, int& luck)
{
    ...
    log::info(std::string(row::k_underline_length + 10, '-'));
}
...
void l_path_from1_to42(Path& path, Inventory& inventory)
{
    ...
    path.add_step(Step{"to roses", 2, e_condition});
    path.add_step(Step{"pick up a white rose", 615});
    path.show_title();
    path.show(1, 615);
    path.show_title("surprise:");
    inventory.add_item(game_items.at(item::k_guiding_thread_name));

    path.add_step(
        Step{"go down the alley", 446,
             e_condition});
    path.add_step(
        Step{"eavesdropping on the conversation", 116,
             e_condition});
    path.show(446, 116);
    path.add_step(Step{"pretending to be a gardener", 217});
    path.add_step(Step{"the guard leaves", 174});
    path.add_step(
        Step{"continue to walk along the alley", 42,
             e_condition});
    path.show_title("\n->");
    path.show(217, 42);
    path.break_line();
...
void l_path_from42_to254(Path& path, Inventory& inventory)
{
    ...
    path.show_title("surprise:");
    inventory.add_item(game_items.at(item::k_fish_bone_name));
    inventory.add_item(game_items.at(item::k_handkerchief_name));

    path.add_step(
        Step{"inside the wardrobe", 352,
             e_condition});
    path.add_step(
        Step{"looking at the costumes", 112,
             e_condition});
    path.show_title("->");
    path.show(352, 112);
    // I don't take outfits but only a small mirror
    path.show_title("surprise:\n");
    inventory.add_item(game_items.at(item::k_small_mirror_name));

    path.add_step(Step{"open the door in the wall", 22});
    path.add_step(
        Step{"stand on the balcony", 254,
             e_condition});
    path.show_title("->");
    path.show(22, 254);
    path.break_line();
...
void l_path_from254_to524(Path& path)
{
    ...
    path.add_step(Step{"whispers stop", 524, e_condition});
    path.show_title("continue:");
    path.show(80, 524);
    path.break_line();
}
...
void l_path_from375_to177(Path& path, Player& player)
{
    ...
    path.add_step(
        Step{"to the Duke's residence", 109,
             e_condition});
    path.add_step(
        Step{"invitation to lunch", 357,
             e_condition});
...
void l_path_from309_to6(Path& path, Player& player)
{
    ...
    path.add_step(Step{"fight with Mortimer", 6, e_fight});
    path.show(309, 6);
    path.break_line();
...
void l_path_from6_to529(Path& path, Inventory& inventory, Player& player)
{
    ...
    path.add_step(
        Step{"down into the corridor", 8,
             e_condition});
    path.add_step(
        Step{"go down the corridor", 121,
             e_condition});
    path.add_step(Step{"left", 381, e_condition});
    path.add_step(
        Step{"in the sanctuary", 247,
             e_condition});
    path.add_step(
        Step{"take a helmet and fight begins", 529,
             e_fight});
    path.show_title("->");
    path.show(8, 529);
    path.break_line();
...
void game_play_death3()
{
    ...
    path.add_step(Step{"there is silence", 43, e_final});
    path.show(314, 43);
...
void game_play_death2()
{
    ...
    path.add_step(Step{"go down the stairs", 43, e_final});
    path.show(43);
